    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (leaguesToCheck.length === 0) {
                ...
                const soccerLeagues = sports.filter(sport => sport.key.includes('soccer') || sport.group.toLowerCase().includes('soccer'));
            ...
            for (const league of leaguesToCheck) {
                ...
                for (const region of regions) {
                    try {
                        const odds = yield client.getOdds(league.key, { regions: region });
                        ...
                        if (!bookmakersByRegion[region]) {
                            bookmakersByRegion[region] = new Set();
                        }
            ...
            for (const region of regions) {
                ...
                bookmakers.forEach(key => {
                    var _a;
                    console.log(`  - ${((_a = bookmakerDetails[key]) === null || _a === void 0 ? void 0 : _a.title) || key} (${key})`);
                });
            }
            //Check for specific bookmakers
            console.log('\n\n==== SPECIFIC BOOKMAKER CHECK ====');
            const checkForBookmaker = (partialName) => {
                const matches = [];
            ...
            //Save to a JSON file
            const results = {
                bookmakersByRegion: Object.fromEntries(Object.entries(bookmakersByRegion).map(([region, bookmakers]) => [region, Array.from(bookmakers)])),
                bookmakerDetails,
