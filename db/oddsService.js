...
class OddsService {
    saveGame(game) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const existingGameResult = yield client.query(`SELECT id, game_id, commence_time FROM games 
                ...
                if (existingGameResult.rows.length > 0) {
                    ...
                    if (hoursDiff <= 24) {
                        ...
                        yield client.query(`UPDATE games SET commence_time = $1 WHERE id = $2`, [game.commence_time, gameId]);
                    }
                }
                else {
                    // No existing game found, insert new game
                    const gameResult = yield client.query(`INSERT INTO games (game_id, sport_key, sport_title, commence_time, home_team, away_team)
                     VALUES ($1, $2, $3, $4, $5, $6)
                ...
                for (const bookmaker of game.bookmakers) {
                    ...
                    const bookmakerResult = yield client.query(`INSERT INTO bookmakers (bookmaker_key, title, last_update)
                     VALUES ($1, $2, $3)
                    ...
                    if (h2hMarket) {
                        ...
                        if (homeWinOutcome && awayWinOutcome && drawOutcome) {
                            ...
                            yield client.query(`INSERT INTO match_odds 
    ...
    getBestOddsForMatch(homeTeam, awayTeam) {
        return __awaiter(this, void 0, void 0, function* () {
